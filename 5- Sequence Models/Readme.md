...
- Lets build a RNN that solves **name entity recognition** task:<br/>
    ![](Images/02.png)
...
- A lot of papers and books write the same architecture this way:<br/>
  ![](Images/03.png)
...
- Now let's discuss the forward propagation equations on the discussed architecture:<br/>
    ![](Images/04.png)
...
- We will use the cross-entropy loss function:<br/>
  ![](Images/07.png)
...
- The ideas in this section was inspired by Andrej Karpathy [blog](http://karpathy.github.io/2015/05/21/rnn-effectiveness/). Mainly this image has all types:<br/>
  ![](Images/09.jpg)
- The architecture we have descried before is called **Many to Many**.
- In sentiment analysis problem, X is a text while Y is an integer that rangers from 1 to 5. The RNN architecture for that is **Many to One** as in Andrej Karpathy image.<br/>
  ![](Images/10.png)
- A **One to Many** architecture application would be music generation.<br/>
  ![](Images/11.png)
  - Note that starting the second layer we are feeding the generated output back to the network.
- There are another interesting architecture in **Many To Many**. Applications like machine translation inputs and outputs sequences have different lengths in most of the cases. So an alternative _Many To Many_ architecture that fits the translation would be as follows:<br/>
  ![](Images/12.png)
...
- Given the sentence "Cats average 15 hours of sleep a day. `<EOS>`"
  - In training time we will use this:<br/>
    ![](Images/13.png)
  - The loss function is defined by cross-entropy loss:<br/>
    ![](Images/14.png)
...
- As we have discussed in Deep neural networks, deeper networks are getting into the vanishing gradient problem. That also happens with RNNs with a long sequence size.<br/>
  ![](Images/16.png)
...
- The basic RNN unit can be visualized to be like this:<br/>
  ![](Images/17.png)
...
- Here are the equations of an LSTM unit:<br/>
  ![](Images/21.png)
...
- Drawings (inspired by http://colah.github.io/posts/2015-08-Understanding-LSTMs/):<br/>
  ![](Images/22.png)
...
- As we saw before, here is an example of the Name entity recognition task:<br/>
  ![](Images/23.png)
- The name **Teddy** cannot be learned from **He** and **said**, but can be learned from **bears**.
- BiRNNs fixes this issue.
- Here is BRNNs architecture:<br/>
  ![](Images/24.png)
...
- For example, a deep RNN with 3 layers would look like this:<br/>
  ![](Images/25.png)
...
- So, instead of a one-hot presentation, won't it be nice if we can learn a featurized representation with each of these words: man, woman, king, queen, apple, and orange?<br/>
  ![](Images/28.png)
...
- To visualize word embeddings we use a t-SNE algorithm to reduce the features to 2 dimensions which makes it easy to visualize:<br/>
  ![](Images/29.png)
...
- Given this example (from named entity recognition):<br/>
  ![](Images/30.png)
...
- Word embeddings have an interesting relationship to the face recognition task:<br/>
  ![](Images/31.png)
...
- Analogies example:
  - Given this word embeddings table:<br/>
    ![](Images/32.png)
  ...
  - So the difference is about the gender in both.<br/>
    ![](Images/33.png)
  ...
  - It can also be represented mathematically by:<br/>
    ![](Images/34.png)
...
- Let's take an example:
  ...
  - The algorithm should create a matrix `E` of the shape (300, 10000) in case we are extracting 300 features.<br/>
    ![](Images/36.png)
...
- **<u>Neural language model</u>**:
  ...
  - So we use this neural network to learn the language model<br/>
    ![](Images/38.png)
...
- One of the solutions for the last problem is to use "**Hierarchical softmax classifier**" which works as a tree classifier.<br/>
  ![](Images/40.png)
...
- Now let's define the model that will learn this supervised learning problem:
  ...
  ![](Images/41.png)
  - The logistic regression model can be drawn like this:<br/>
  ![](Images/42.png)
...
- How to select negative samples:
  ...
  - The best is to sample with this equation (according to authors):<br/>
    ![](Images/43.png)
...
- The model is defined like this:<br/>
  ![](Images/44.png)
...
- As we have discussed before, Sentiment classification is the process of finding if a text has a positive or a negative review. Its so useful in NLP and is used in so many applications. An example would be:<br/>
  ![](Images/45.png)
...
- A simple sentiment classification model would be like this:<br/>
  ![](Images/46.png)
...
- A better model uses an RNN for solving this problem:<br/>
  ![](Images/47.png)
...
- Addressing bias in word embeddings steps:
  ...
  - Given these learned embeddings:<br/>
    ![](Images/48.png)
  ...
  - Here are the steps:
    1. Identify the direction:
       ...
       - This will help you find this:<br/>
         ![](Images/49.png)
       - By that we have found the bias direction which is 1D vector and the non-bias vector which is 299D vector.
    2. Neutralize: For every word that is not definitional, project to get rid of bias.
       - Babysitter and doctor need to be neutral so we project them on non-bias axis with the direction of the bias:<br/>
         ![](Images/50.png)
 ...
         - He - She
         - Boy - Girl
       - We want to do this because the distance between grandfather and babysitter is bigger than babysitter and grandmother:<br/>
         ![](Images/51.png)
...
- Let's start with the basic model:
  - Given this machine translation problem in which X is a French sequence and Y is an English sequence.<br/>
    ![](Images/52.png)
  ...
  - After that the decoder network, also RNN, takes the sequence built by the encoder and outputs the new sequence.<br/>
  ![](Images/53.png)
...
- An architecture similar to the mentioned above works for image captioning problem:
  ...
  - The model architecture image:<br/>
    ![](Images/54.png)
...
- The language model we have learned is very similar to the decoder part of the machined translation model, except for a<sup>\<0></sup><br/>
  ![](Images/55.png)
...
- So we need to get the best output it can be:<br/>
  ![](Images/56.png)
...
- The first thing is **Length optimization**
  - In beam search we are trying to optimize:<br/>
    ![](Images/56.png)
  ...
  - So in practice we use **summing logs of probabilities** instead of multiplying directly.<br/>
    ![](Images/57.png)
  ...
  - So there's another step - dividing by the number of elements in the sequence.<br/>
    ![](Images/58.png)
...
- The error analysis process is as following:
  - You choose N error examples and make the following table:<br/>
    ![](Images/59.png)
...
- So here are the equations for modified presicion for the n-grams case:<br/>
  ![](Images/60.png)
- Let's put this together to formalize the BLEU score:
  ...
  - **BP** is called **BP penalty** which stands for brevity penalty. It turns out that if a machine outputs a small number of words it will get a better score so we need to handle that.<br/>
    ![](Images/62.png)
...
- The problem of long sequences:
  - Given this model, inputs, and outputs.<br/>
    ![](Images/63.png)
  ...
  - We will discuss the attention model that works like a human that looks at parts at a time. That will significantly increase the accuracy even with longer sequence:<br/>
    ![](Images/64.png)
...
- Now for the intuition:
  - Suppose that our encoder is a bidirectional RNN:<br/>
    ![](Images/65.png)
  ...
  - Attention weights are used to specify which words are needed when to generate a word. So to generate "jane" we will look at "jane", "visite", "l'Afrique"<br/>
    ![](Images/66.png)
  - alpha<sup>\<1,1></sup>, alpha<sup>\<1,2></sup>, and alpha<sup>\<1,3></sup> are the attention weights being used.
  - And so to generate any word there will be a set of attention weights that controls which words we are looking at right now.<br/>
    ![](Images/67.png)

#### Attention Model
- Lets formalize the intuition from the last section into the exact details on how this can be implemented.
- First we will have an bidirectional RNN (most common is LSTMs) that encodes French language:<br/>
  ![](Images/68.png)
- For learning purposes, lets assume that a<sup>\<t'></sup> will include the both directions activations at time step t'.
- We will have a unidirectional RNN to produce the output using a context `c` which is computed using the attention weights, which denote how much information does the output needs to look in a<sup>\<t'></sup><br/>
  ![](Images/69.png)
- Sum of the attention weights for each element in the sequence should be 1:<br/>
  ![](Images/70.png)
- The context `c` is calculated using this equation:<br/>
  ![](Images/71.png)
- Lets see how can we compute the attention weights:
  ...
  - We are going to softmax the attention weights so that their sum is 1:<br/>
    ![](Images/72.png)
  - Now we need to know how to calculate e<sup>\<t, t'></sup>. We will compute e using a small neural network (usually 1-layer, because we will need to compute this a lot):<br/>
    ![](Images/73.png)
...
- One fun way to see how attention works is by visualizing the attention weights:<br/>
  ![](Images/74.png)
...
- Let's define the speech recognition problem:
  - X: audio clip
  - Y: transcript
  - If you plot an audio clip it will look like this:<br/>
    ![](Images/75.png)
  ...
  - There's a common preprocessing step for an audio - generate a spectrogram which works similarly to human ears.<br/>
    ![](Images/76.png)
...
- You can build an accurate speech recognition system using the attention model that we have descried in the previous section:<br/>
  ![](Images/77.png)
- One of the methods that seem to work well is _CTC cost_ which stands for "Connectionist temporal classification"
  ...
  - We are going to use an RNN with input, output structure:<br/>
    ![](Images/78.png)
...
- Trigger word detection systems include:<br/>
  ![](Images/79.png)
...
- Let's now build a model that can solve this problem:
  ...
  - The model architecture can be like this:<br/>
    ![](Images/80.png)
  ...
  - A hack to solve this is to make an output a few ones for several times or for a fixed period of time before reverting back to zero.<br/>
    ![](Images/81.png)
    ![](Images/85.png)
...
- What one "Attention" step does to calculate the attention variables &alpha;<sup>`<t, t>`</sup>, which are used to compute the context variable context<sup>`<t>`</sup> for each timestep in the output (t=1, ..., T<sub>y</sub>).<br/>
  ![](Images/84.png)
